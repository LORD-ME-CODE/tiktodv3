...
def title1():  # Update the title IF option 1 was picked.
    ...
    while True:
        time_elapsed = strftime("%H:%M:%S", gmtime(time() - start))
        system(
            f"title TIKTOD V3 ^| Views Sent: {beautify(Views)} ^| Elapsed Time: {time_elapsed}"
        )
...
def title2():  # Update the title IF option 2 was picked.
    ...
    while True:
        time_elapsed = strftime("%H:%M:%S", gmtime(time() - start))
        system(
            f"title TIKTOD V3 ^| Hearts Sent: {beautify(Hearts)} ^| Elapsed Time: {time_elapsed}"
        )
...
def title3():  # Update the title IF option 3 was picked.
    ...
    while True:
        time_elapsed = strftime("%H:%M:%S", gmtime(time() - start))
        system(
            f"title TIKTOD V3 ^| Followers Sent: {beautify(Followers)} ^| Elapsed Time: {time_elapsed}"
        )
...
def title4():  # Update the title IF option 1 was picked.
    ...
    while True:
        time_elapsed = strftime("%H:%M:%S", gmtime(time() - start))
        system(
            f"title TIKTOD V3 ^| Shares Sent: {beautify(Shares)} ^| Elapsed Time: {time_elapsed}"
        )
...
def loop1():
    ...
    try:
        sleep(2)
        driver.find_element_by_xpath('//*[@id="sid4"]/div/form/div/input').send_keys(
            vidUrl
        )
...
def loop2():
    ...
    try:
        sleep(2)
        driver.find_element_by_xpath('//*[@id="sid2"]/div/form/div/input').send_keys(
            vidUrl
        )
        ...
        sleep(6)
        hearts = driver.find_element_by_xpath(
            '//*[@id="c2VuZE9nb2xsb3dlcnNfdGlrdG9r"]/span'
        ).text.split()
...
def loop3():
    ...
    try:
        sleep(2)
        driver.find_element_by_xpath('//*[@id="sid"]/div/form/div/input').send_keys(
            vidUrl
        )
        ...
        sleep(6)
        folls = driver.find_element_by_xpath(
            '//*[@id="c2VuZF9mb2xsb3dlcnNfdGlrdG9r"]/span'
        ).text.split()
...
def loop4():
    ...
    try:
        sleep(2)
        driver.find_element_by_xpath('//*[@id="sid7"]/div/form/div/input').send_keys(
            vidUrl
        )
